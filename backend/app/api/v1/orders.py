...
async def update_order_status(
    ...
    if order.payment_status == PaymentStatus.PENDING.value:
        if status_data.status == OrderStatus.CONFIRMED and order.payment_method in ["bkash", "card"]:
            order.payment_status = PaymentStatus.COMPLETED.value
        elif status_data.status == OrderStatus.DELIVERED and order.payment_method == "cod":
            order.payment_status = PaymentStatus.COMPLETED.value
...
@router.put("/{order_number}/payment-status", response_model=OrderResponse)
async def update_payment_status(
    ...
    # Add tracking entry for payment status change
    tracking = OrderTracking(
        order_id=order.id,
        status=f"Payment {payment_status.value.title()}",
        description=f"Payment status updated to {payment_status.value}"
    )
    db.add(tracking)
